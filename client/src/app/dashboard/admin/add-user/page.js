...
import { useAddUserMutation } from "@/services/user/userApi"; // Changed from authApi
...
function AddUser() {
  ...
  const [addUser, { isLoading, data, error }] = useAddUserMutation(); // Changed from useSignUpMutation
  const router = useRouter();

  useEffect(() => {
    if (isLoading) {
      toast.loading("Creating User...", { id: "addUser" });
    }
  ...
  return (
    <form className="w-full flex flex-col gap-y-4" onSubmit={handleAddUser}>
      {/* Avatar */}
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-yellow-800">
                <strong>Admin users</strong> don't require a phone number for account creation.
              </span>
            </div>
          </div>
        )}
      ...
      >
        {isLoading ? "Creating User..." : `Create ${selectedRole === 'admin' ? 'Admin' : 'User'}`}
      </button>
    </form>
  );
}
