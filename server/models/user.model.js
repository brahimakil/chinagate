...
const userSchema = new mongoose.Schema(
  {
    ...
    phone: {
      type: String,
      required: function() {
        // Phone is only required for non-admin users
        return this.role !== "admin";
      },
      validate: {
        validator: function(value) {
          // If no value and role is admin, it's valid
          if (!value && this.role === "admin") {
            return true;
          }
          // For non-admin users or when value is provided, validate format
          const phoneRegex = /^\+\d{7,15}$/;
          return phoneRegex.test(value);
        },
        message:
          "Phone number {VALUE} is not valid. Please provide a valid international phone number with country code (e.g., +1234567890)",
      },
      unique: true,
      sparse: true, // This allows multiple null values (for admin users)
    },
